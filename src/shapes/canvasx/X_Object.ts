...
export class FabricObject2 {
  ...
  id = '';
  idsInGroup;
  activeSelectWithoutArrow;
  activeSelectionWithArrow;
  ...
  convertRCoordToACoordPartialAS(rx, ry) {
    const target = this;
    ...
    if (obj && obj.group) {
      ...
      const ay =
        (-rx * Math.sin(angle) + ry * Math.cos(angle)) * (height * scaleY) +
        top;
  ...
  async saveData(action, fields) {
    ...
    if (target.isActiveSelection()) {
      ...
      for (const obj of target.getObjects()) {
        if (obj._id) {
          this.idsInGroup.push(obj._id);

          if (obj.obj_type === 'WBArrow') {
            this.activeSelectionWithArrow.push(obj._id);
          }

          if (obj.obj_type !== 'WBArrow') {
            this.activeSelectWithoutArrow.push(obj._id);
          }
  ...
  getCloneWidget() {
    ...
    if (widget.connectorEnd)
      widget.connectorEnd = { _id: this.connectorEnd._id };
    delete widget._id;
    delete widget.emoji;
    widget.isPanel = null;
