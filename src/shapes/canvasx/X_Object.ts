...
FabricObject.prototype.isRemoteEditing = false;
FabricObject.prototype.RemoteUserWhoEditing = '';
FabricObject.prototype.NUM_FRACTION_DIGITS = 2;
FabricObject.prototype.activeOn = 'down';
FabricObject.prototype.borderColor = 'red';
FabricObject.prototype.cornerSize = 12;
FabricObject.prototype.hasRotatingPoint = false;
FabricObject.prototype.minScaleLimit = 0.01;
...
FabricObject.prototype.containsPointNew = function (point, ismouseup, isFrom) {
  const self = this;
...
FabricObject.prototype.frameExtentContainsPoint = function (point) {
  const self = this;
...
FabricObject.prototype.convertACoordToRCoord = function (ax, ay) {
  const target = this;
  ...
  const rx =
    (deltaX * Math.cos(angleT) + deltaY * Math.sin(angleT)) / (width * scaleX);
...
FabricObject.prototype.convertRCoordToACoord1 = function (rx, ry) {
  const target = this;
...
FabricObject.prototype.convertRCoordToACoord = function (rx, ry) {
  const target = this;
  ...
  const ay =
    (-rx * Math.sin(angle1) + ry * Math.cos(angle1)) * (height * scaleY) + top;
...
FabricObject.prototype.convertRCoordToACoordPartialAS = function (rx, ry) {
  const target = this;
  ...
  if (obj && obj.group) {
    ...
    const ax =
      (rx * Math.cos(angle) + ry * Math.sin(angle)) * (width * scaleX) + left;

    const ay =
      (-rx * Math.sin(angle) + ry * Math.cos(angle)) * (height * scaleY) + top;
...
FabricObject.prototype.saveData = async function (action, fields) {
  const target = this;
...
FabricObject.prototype.initialize = function (options) {
  const self = this;
  this.guides = {};
  if (options) {
...
FabricObject.prototype.getCloneWidget = function () {
...
FabricObject.prototype.bringObjToFront = function () {
  const obj = this;
...
FabricObject.prototype.sendObjToBack = function () {
  const obj = this;
  let newIndex;
  const intersectingObjects = canvas._getIntersectedObjects(obj);
...
FabricObject.prototype.bringObjForward = function () {
  const obj = this;
  const intersectingObjects = canvas._getIntersectedObjects(obj);
  if (intersectingObjects.length > 1) {
...
FabricObject.prototype.sendObjBackwards = function () {
  const obj = this;
  const intersectingObjects = canvas._getIntersectedObjects(obj);
  intersectingObjects.sort((a, b) => a.zIndex - b.zIndex);
  const index = intersectingObjects.indexOf(obj);
...
FabricObject.prototype.reSetControl = function () {
  const obj = this;
...
FabricObject.prototype.drawBordersInGroup = function (
  ctx,
  options,
  styleOverride
) {
...
//  * @chainable
//  */
// FabricObject.prototype.drawBorders = function (ctx, styleOverride) {
//   styleOverride = styleOverride || {};
...
//   return this;
// };
FabricObject.prototype.isActiveSelection = function () {
  const obj = this;
...
FabricObject.prototype.isGroup = function () {
  const obj = this;
